...
pub fn rank(alert: &Alert, runbooks: &[Runbook], top_n: usize) -> Vec<ScoredRunbook> {
    ...
    let mut scored: Vec<ScoredRunbook> = runbooks
        ...
        .map(|rb| {
            ...
            let title_hits = alert_tokens.iter().filter(|t| title_tokens.contains(t)).count();
            let body_hits = alert_tokens.iter().filter(|t| body_tokens.contains(t)).count();
            ScoredRunbook {
                path: rb.path.clone(),
...
mod tests {
    ...
    fn runbook(path: &str, title: &str, content: &str) -> Runbook {
        Runbook { path: path.into(), title: title.into(), content: content.into() }
    }
